...
- **MaxDD の悪化がテール期間外で発生** — slope 5.2 で MaxDD 51.4% に対しテール期間 DD は 16.4%。
  原因は DD hard stop のピークリセット（KOH-567）。危機局面ではなく平常時の階段状の減価
- **真のハードストップ下では baseline も不合格** — DD stop がピークをリセットしない実装で
  再計測すると baseline CAGR 6.42% < 閾値 6.5%。現状の合格は「損失を忘れて取引を再開する」
  挙動にも支えられていた（ただし stuck state による機会損失分との切り分けは未了）
