// scripts/walk-forward/walk-forward-us-iron-condor.ts
/**
 * SPY Iron Condor Walk-Forward 分析
 *
...
import { runUSIronCondorBacktest } from "../../src/backtest/us/us-iron-condor-simulation";
import {
  US_IRON_CONDOR_DEFAULTS,
  US_IRON_CONDOR_BACKTEST_FIDELITY,
  generateUSIronCondorParameterCombinations,
  US_IRON_CONDOR_PARAMETER_GRID,
} from "../../src/backtest/us/us-iron-condor-config";
import type { USIronCondorBacktestConfig } from "../../src/backtest/us/us-iron-condor-types";
import type { PerformanceMetrics } from "../../src/backtest/types";
import { summarizeWFResults, emitJsonSummary, isJsonMode } from "./lib/wf-summary";
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<USIronCondorBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<USIronCondorBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(p: Partial<USIronCondorBacktestConfig>): string {
  return `sd${p.shortDelta}_dte${p.dte}_pt${p.profitTarget}`;
}
...
function selectByRobustness(combos: Map<string, ComboResult>): ComboResult | null {
  const grids: readonly number[][] = [
    [...US_IRON_CONDOR_PARAMETER_GRID.shortDelta],
    [...US_IRON_CONDOR_PARAMETER_GRID.dte],
    [...US_IRON_CONDOR_PARAMETER_GRID.profitTarget],
  ];
  ...
  for (const r of combos.values()) {
    ...
    function recurse(dim: number, cur: number[]): void {
      if (dim === ranges.length) {
        ...
        const k = `sd${vals[0]}_dte${vals[1]}_pt${vals[2]}`;
...
async function main() {
  ...
  const creditScale = csIdx >= 0 && csIdx + 1 < process.argv.length
    ...
    : (US_IRON_CONDOR_DEFAULTS.creditScale ?? 1);

  console.log("=".repeat(70));
  console.log("SPY Iron Condor Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  console.log(`Skew slope: put=${US_IRON_CONDOR_BACKTEST_FIDELITY.putSkewSlope} / call=${US_IRON_CONDOR_BACKTEST_FIDELITY.callSkewSlope}`);
  console.log(`Credit Scale: ${creditScale.toFixed(3)} (1.0 = BS理論どおり)`);

  const paramCombos = generateUSIronCondorParameterCombinations();
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const oosConfig: USIronCondorBacktestConfig = {
      ...US_IRON_CONDOR_DEFAULTS,
      ...US_IRON_CONDOR_BACKTEST_FIDELITY,
      ...bestParams,
      startDate: oosStart,
    ...
    const oosResult = await runUSIronCondorBacktest(oosConfig, gspc, vix);
    ...
    console.log(`  最適パラメータ: shortDelta=${bestParams.shortDelta}, dte=${bestParams.dte}, profitTarget=${bestParams.profitTarget}\n`);
  }

  if (isJsonMode()) {
    const summary = summarizeWFResults("us-iron-condor", results, {
      isMonths: IS_MONTHS,
...
function printSummary(results: WindowResult[]) {
  console.log("=".repeat(70));
  console.log("SPY Iron Condor Walk-Forward サマリー");
  console.log("=".repeat(70));
  ...
  for (const r of results) {
    ...
    const ps = `sd=${p.shortDelta}, dte=${p.dte}, pt=${p.profitTarget}`;
  ...
  console.log("\n[パラメータ安定性]");
  for (const key of ["shortDelta", "dte", "profitTarget"] as const) {
