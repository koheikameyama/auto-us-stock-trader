// scripts/walk-forward-us-momentum.ts
/**
 * Momentum (US) Walk-Forward 分析
 *
...
import {
  runUSMomentumBacktest,
  precomputeUSMomentumDailySignals,
} from "../../src/backtest/us/us-momentum-simulation";
import {
  US_MOMENTUM_DEFAULTS,
  generateUSMomentumParameterCombinations,
  US_MOMENTUM_PARAMETER_GRID,
} from "../../src/backtest/us/us-momentum-config";
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<USMomentumBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<USMomentumBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<USMomentumBacktestConfig>): string {
...
function selectByRobustness(comboResults: Map<string, ComboResult>): ComboResult | null {
  const gridArrays: number[][] = [
    [...US_MOMENTUM_PARAMETER_GRID.atrMultiplier],
    [...US_MOMENTUM_PARAMETER_GRID.beActivationMultiplier],
    [...US_MOMENTUM_PARAMETER_GRID.trailMultiplier],
  ];
...
async function main() {
  ...
  console.log("Momentum (US) Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  const paramCombos = generateUSMomentumParameterCombinations();
  ...
  const maxPrice = US_MOMENTUM_DEFAULTS.maxPrice;
  ...
  const filterCfg = US_MOMENTUM_DEFAULTS;
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const isSignals = precomputeUSMomentumDailySignals(filterCfg, allData, isPrecomputed);
    ...
    for (const params of paramCombos) {
      ...
      const result = runUSMomentumBacktest(config, allData, vixArg, indexArg, isPrecomputed, isSignals);
    ...
    const oosSignals = precomputeUSMomentumDailySignals(filterCfg, allData, oosPrecomputed);

    const oosConfig: USMomentumBacktestConfig = {
      ...US_MOMENTUM_DEFAULTS,
      ...bestParams,
      startDate: oosStart,
      endDate: oosEnd,
    ...
    const oosResult = runUSMomentumBacktest(oosConfig, allData, vixArg, indexArg, oosPrecomputed, oosSignals);
