// scripts/walk-forward-us-gapup.ts
/**
 * GapUp (US) Walk-Forward 分析
 *
 ...
 *   npm run walk-forward:us-gapup -- --max-pf
 */

import dayjs from "dayjs";
import { prisma } from "../../src/lib/prisma";
import { getUSTickerCodes, fetchUSHistoricalFromDB, fetchSP500FromDB, fetchVixFromDB } from "../../src/backtest/us/us-data-fetcher";
import { precomputeUSSimData } from "../../src/backtest/us/us-simulation-helpers";
import { runUSGapUpBacktest, precomputeUSGapUpDailySignals } from "../../src/backtest/us/us-gapup-simulation";
import { US_GAPUP_DEFAULTS, generateUSGapUpParameterCombinations, US_GAPUP_PARAMETER_GRID, US_GAPUP_RELAXED_GAP_MIN_PCT } from "../../src/backtest/us/us-gapup-config";
import type { USGapUpBacktestConfig } from "../../src/backtest/us/us-types";
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<USGapUpBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<USGapUpBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<USGapUpBacktestConfig>): string {
...
function selectByRobustness(comboResults: Map<string, ComboResult>): ComboResult | null {
  const gapRelaxValues = [...US_GAPUP_PARAMETER_GRID.gapRelaxVolThreshold];
  const gridArrays: (number | undefined)[][] = [
    gapRelaxValues,
    [...US_GAPUP_PARAMETER_GRID.atrMultiplier],
    [...US_GAPUP_PARAMETER_GRID.beActivationMultiplier],
    [...US_GAPUP_PARAMETER_GRID.trailMultiplier],
  ];
...
async function main() {
  ...
  console.log("GapUp (US) Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  const paramCombos = generateUSGapUpParameterCombinations();
  ...
  console.log(`[data] ${rawData.size}銘柄（raw）, VIX ${vixData.size}日, S&P500 ${sp500Data.size}日`);

  const maxPrice = US_GAPUP_DEFAULTS.maxPrice;
  ...
  const filterCfg = signalSortMethod
    ? { ...US_GAPUP_DEFAULTS, signalSortMethod }
    : US_GAPUP_DEFAULTS;
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    // IS 事前計算（precomputeUSSimData は共通、signals は gapRelaxVolThreshold ごとに分ける）
    ...
    const isSignalsByRelax = new Map(
      US_GAPUP_PARAMETER_GRID.gapRelaxVolThreshold.map((threshold) => [
        threshold ?? "off",
        precomputeUSGapUpDailySignals(
          {
            ...filterCfg,
            gapRelaxVolThreshold: threshold,
            gapMinPctRelaxed: threshold != null ? US_GAPUP_RELAXED_GAP_MIN_PCT : undefined,
          },
          allData,
          isPrecomputed,
        ),
    ...
    for (const params of paramCombos) {
      ...
      const config: USGapUpBacktestConfig = {
        ...US_GAPUP_DEFAULTS,
        ...params,
        startDate: isStart,
        endDate: isEnd,
      ...
      const result = runUSGapUpBacktest(config, allData, vixArg, indexArg, isPrecomputed, isSignals);
    ...
    const oosSignals = precomputeUSGapUpDailySignals(
      {
        ...filterCfg,
    ...
    const oosConfig: USGapUpBacktestConfig = {
      ...US_GAPUP_DEFAULTS,
      ...bestParams,
      startDate: oosStart,
      endDate: oosEnd,
    ...
    const oosResult = runUSGapUpBacktest(oosConfig, allData, vixArg, indexArg, oosPrecomputed, oosSignals);
  ...
  if (isJsonMode()) {
    const summary = summarizeWFResults("us-gapup", results, {
      isMonths: IS_MONTHS,
...
function printSummary(results: WindowResult[]): void {
  ...
  for (const r of results) {
    ...
    const relaxStr = p.gapRelaxVolThreshold != null
      ...
      : "no-relax";
    const paramStr = `gap=3%(${relaxStr}), atr=${p.atrMultiplier}, be=${p.beActivationMultiplier}, trail=${p.trailMultiplier}`;
