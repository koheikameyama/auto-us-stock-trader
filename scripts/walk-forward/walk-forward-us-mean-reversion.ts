// scripts/walk-forward-us-mean-reversion.ts
/**
 * Mean Reversion (US) Walk-Forward 分析
 *
...
import { runUSMeanReversionBacktest, precomputeUSMeanReversionDailySignals } from "../../src/backtest/us/us-mean-reversion-simulation";
import {
  US_MEAN_REVERSION_DEFAULTS,
  generateUSMeanReversionParameterCombinations,
  US_MEAN_REVERSION_PARAMETER_GRID,
} from "../../src/backtest/us/us-mean-reversion-config";
import type { USMeanReversionBacktestConfig } from "../../src/backtest/us/us-types";
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<USMeanReversionBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<USMeanReversionBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<USMeanReversionBacktestConfig>): string {
...
function selectByRobustness(comboResults: Map<string, ComboResult>): ComboResult | null {
  const gridArrays: number[][] = [
    [...US_MEAN_REVERSION_PARAMETER_GRID.atrMultiplier],
    [...US_MEAN_REVERSION_PARAMETER_GRID.beActivationMultiplier],
    [...US_MEAN_REVERSION_PARAMETER_GRID.trailMultiplier],
  ];
...
async function main() {
  ...
  console.log("Mean Reversion (US) Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  const paramCombos = generateUSMeanReversionParameterCombinations();
  ...
  console.log(`[data] ${rawData.size}銘柄（raw）, VIX ${vixData.size}日, S&P500 ${indexData.size}日`);

  const maxPrice = US_MEAN_REVERSION_DEFAULTS.maxPrice;
  ...
  const filterCfg = US_MEAN_REVERSION_DEFAULTS;
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const isSignals = precomputeUSMeanReversionDailySignals(filterCfg, allData, isPrecomputed);
    ...
    for (const params of paramCombos) {
      ...
      const result = runUSMeanReversionBacktest(config, allData, vixArg, indexArg, isPrecomputed, isSignals);
    ...
    const oosSignals = precomputeUSMeanReversionDailySignals(filterCfg, allData, oosPrecomputed);

    const oosConfig: USMeanReversionBacktestConfig = {
      ...US_MEAN_REVERSION_DEFAULTS,
      ...bestParams,
      startDate: oosStart,
      endDate: oosEnd,
    ...
    const oosResult = runUSMeanReversionBacktest(oosConfig, allData, vixArg, indexArg, oosPrecomputed, oosSignals);
  ...
  if (isJsonMode()) {
    const summary = summarizeWFResults("us-mean-reversion", results, {
      isMonths: IS_MONTHS,
...
function printSummary(results: WindowResult[]): void {
  console.log("=".repeat(70));
  console.log("Mean Reversion (US) Walk-Forward サマリー");
  console.log("=".repeat(70));
