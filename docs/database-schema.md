...
|---|---|---|---|
| 名前衝突 | なし（`us.StockDailyBar` と `public.StockDailyBar` 共存可）| なし | あり（`us_stock_daily_bar` 冗長） |
...
schema が `us` で名前空間分離されているので `us.StockDailyBar` でJP側と衝突しない。
`us.USStockDailyBar` は冗長（`/us/us-stock-daily-bar`）。
...
| テーブル | 内容 |
|---|---|
| `us.TradingOrder` | 注文履歴（買・売、現物・オプション）|
| `us.Position` | 保有ポジション |
| `us.OptionContract` | オプション契約（Credit Spread 用、strike/expiry/delta/IV等）|
| `us.MarketAssessment` | 市場評価（VIXレジーム、SPYトレンド等）|

## Prisma 設定（本リポ）

`auto-us-stock-trader` リポに Prisma を導入。
...
```sql
-- 1. us schema にデータ移行
INSERT INTO us."StockDailyBar" (id, "tickerCode", date, open, high, low, close, volume, "createdAt")
SELECT id, "tickerCode", date, open, high, low, close, volume, "createdAt"
FROM public."StockDailyBar"
WHERE market = 'US';

INSERT INTO us."IndexDailyBar" (id, "tickerCode", date, open, high, low, close, volume, "createdAt")
SELECT id, "tickerCode", date, open, high, low, close, volume, "createdAt"
FROM public."StockDailyBar"
WHERE market = 'INDEX';

-- VIX関連ETF/ローテーションETFは「US」にfallbackして入る想定
-- もしSP500/600以外の market="US" もあれば、それも us."StockDailyBar" に流入

-- 2. EarningsDate は public/us 分離が無いので全件移行
INSERT INTO us."EarningsDate" (id, "tickerCode", date, "createdAt")
...
DATABASE_URL="postgresql://kouheikameyama@localhost:5432/auto_stock_trader" \
  npx prisma migrate dev --name init_us_schema

# us schema が作成されたことを確認
psql -U kouheikameyama -h localhost -d auto_stock_trader \
  -c "\\dn"  # schema一覧
psql -U kouheikameyama -h localhost -d auto_stock_trader \
  -c "\\dt us.*"  # us schema のテーブル一覧
```

### Phase 2: backfill スクリプトの書き込み先変更

`scripts/data/*.py` の `INSERT INTO "StockDailyBar"` を `INSERT INTO us."StockDailyBar"` に修正。
...
# 変更後
"""
INSERT INTO us."StockDailyBar" (id, "tickerCode", date, open, high, low, close, volume)
VALUES %s
ON CONFLICT ("tickerCode", date) DO NOTHING
"""
```

`backfill_index.py` は `us."IndexDailyBar"` に変更。

### Phase 3: ローカル動作確認

```bash
# 5スクリプトを順次実行、us schema にデータが入ることを確認
DATABASE_URL=local python scripts/data/backfill_daily_bars.py --index sp500 --yes
# ...

# 行数確認
psql -c "SELECT COUNT(*) FROM us.\"StockDailyBar\";"
```
...
`workflow_dispatch:` で手動 trigger → Railway の us schema に書き込まれることを確認。

### Phase 7: JP側のクリーンアップ（任意、後日）

`auto-stock-trader` 側で `public.StockDailyBar.market="US"` データを削除。
バックテストコードの参照先を us schema に変更（または本リポへ移管）。
...
- **GitHub Actions の Secrets**
  - `DATABASE_URL`: Railway URL を設定。Prisma が自動で us schema を使う
