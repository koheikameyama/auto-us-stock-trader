/**
 * US GapUp バックテスト CLI 実行エントリーポイント
 *
 * Usage:
 *   npx tsx src/backtest/us/us-gapup-run.ts
 *   npx tsx src/backtest/us/us-gapup-run.ts --start 2024-01-01 --end 2025-12-31 --verbose
 */

import dayjs from "dayjs";
import { US_GAPUP_DEFAULTS } from "./us-gapup-config";
import { runUSGapUpBacktest } from "./us-gapup-simulation";
...
async function main() {
  ...
  const budget = getArg("budget") ? Number(getArg("budget")) : US_GAPUP_DEFAULTS.initialBudget;
  ...
  // バックテスト実行
  console.log("Running GapUp backtest...");
  const result = runUSGapUpBacktest(config, allData, vixData, indexData);
