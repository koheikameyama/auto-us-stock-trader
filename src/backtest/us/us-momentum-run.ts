/**
 * Cross-Sectional Momentum バックテスト CLI 実行エントリーポイント
 *
 * Usage:
 *   npx tsx src/backtest/us/us-momentum-run.ts
 *   npx tsx src/backtest/us/us-momentum-run.ts --start 2024-01-01 --end 2025-12-31 --verbose
 */

import dayjs from "dayjs";
import { US_MOMENTUM_DEFAULTS } from "./us-momentum-config";
import { runUSMomentumBacktest } from "./us-momentum-simulation";
...
async function main() {
  ...
  const budget = getArg("budget") ? Number(getArg("budget")) : US_MOMENTUM_DEFAULTS.initialBudget;
  ...
  console.log("Cross-Sectional Momentum Backtest - US");
  console.log("=".repeat(60));
  console.log(`Period: ${startDate} ~ ${endDate}`);
  console.log(`Budget: $${budget.toLocaleString()}`);
  console.log(`Max Positions: ${config.maxPositions}`);
  console.log(`Lookback: ${config.lookbackDays} days | Top N: ${config.topN} | Rebalance: ${config.rebalanceDays} days`);
  console.log();
